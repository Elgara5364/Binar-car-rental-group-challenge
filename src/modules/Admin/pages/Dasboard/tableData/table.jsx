...
const TableData = () => {
  ...
  const getTable = async () => {
    try {
      ...
      setData(
        response.data.orders.map((data, i) => ({
          no: i + 1,
  ...
  const paginationNumberButton = () => {
    ...
    const startPage = Math.max(
      1,
      options.currentPage - Math.floor(visiblePageRange / 2)
    );
    ...
    <>
      ...
      <div className="table-footer">
        <div className="input-groups">
          <div className="input">
            ...
            <select
              ...
              onChange={(e) =>
                setOptions({ ...options, pageSize: Number(e.target.value) })
              }>
          ...
          <div className="input">
            ...
            <select
              name=""
              onChange={(e) =>
                setOptions({ ...options, currentPage: Number(e.target.value) })
              }
              defaultValue={options.currentPage}
              value={options.currentPage}>
              {Array.from(
                { length: Math.ceil(totalRecords / options.pageSize) },
                (_, i) => i + 1
              ).map((page) => (
        ...
        <div className="table-pagination">
          ...
          {options.currentPage >=
          Math.ceil(totalRecords / options.pageSize) - 2 ? null : (
            <button>...</button>
          )}
          <button
            className="number"
            disabled={
              options.currentPage === Math.ceil(totalRecords / options.pageSize)
            }
            onClick={() =>
              handlePagination(Math.ceil(totalRecords / options.pageSize))
            }>
